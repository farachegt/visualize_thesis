...
@dataclass
class DiurnalExtremaMatrixSourceInput(DiurnalSourceInput):
    """Describe one source field participating in an extrema matrix row."""

    pass


@dataclass
class DiurnalExtremaMatrixRowInput:
    """Describe one daily min/max comparison row in a matrix figure."""
...
def plot_diurnal_extrema_matrix_rows(
    *,
    rows: Sequence[DiurnalExtremaMatrixRowInput],
    ...
    """Build a matrix figure with daily min/max maps and their delta."""
    if not rows:
        raise ValueError(
            "At least one DiurnalExtremaMatrixRowInput is required."
        )

    if len(column_headers) != 3:
        raise ValueError(
            "Diurnal extrema matrix requires exactly three "
            "column headers."
        )
    ...
    for row_index, row_input in enumerate(rows):
        row_panels, row_colorbars = _build_diurnal_extrema_matrix_row(
            row_input=row_input,
            row_index=row_index,
        )
...
def _build_diurnal_extrema_matrix_row(
    *,
    row_input: DiurnalExtremaMatrixRowInput,
    row_index: int,
) -> tuple[list[MapPanelInput], list[SharedColorbarSpecification]]:
    """Build the three panels and colorbars associated with one row."""
    left_plot_data = resolve_diurnal_reduced_plot_data(
        row_input.left_source,
        row_input.day_start,
        row_input.day_duration,
        time_reduce=row_input.time_reduce,
    )
    right_plot_data = resolve_diurnal_reduced_plot_data(
    ...
    panels = [
        _build_diurnal_extrema_matrix_panel(
...
def _build_diurnal_extrema_matrix_panel(
    *,
    ...
    """Build one panel of a diurnal-extrema matrix row."""
