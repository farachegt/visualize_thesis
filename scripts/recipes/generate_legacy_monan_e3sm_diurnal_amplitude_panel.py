...
N_DAYS = 3
FIELD_FILE_STEM = "diurnal_amplitude_panel"


def generate_legacy_monan_e3sm_diurnal_amplitude_panel_figures(
    output_dir: Path | None = None,
) -> list[Path]:
    """Generate one legacy-style diurnal-amplitude panel per day."""
    final_output_dir = output_dir or (
        OUTPUT_DIR / "legacy_monan_e3sm_diurnal_amplitude_panel"
    )
    ...
    try:
        for day_start in _build_daily_starts(START_DATE, N_DAYS):
            figure = build_legacy_monan_e3sm_diurnal_amplitude_panel_figure(
                day_start=day_start,
...
def main() -> None:
    """Generate all legacy-style MONAN/E3SM amplitude-panel figures."""
    for output_path in (
        generate_legacy_monan_e3sm_diurnal_amplitude_panel_figures()
    ):
