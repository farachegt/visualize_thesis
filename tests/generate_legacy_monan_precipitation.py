...
START_TIME = np.datetime64("2014-02-24T01:00")
END_TIME = np.datetime64("2014-02-27T00:00")
OUTPUT_SUBDIR = "legacy_monan_precipitation"
FIELD_FILE_STEM = "precipitation"


def generate_legacy_monan_precipitation_figures(
    output_dir: Path | None = None,
) -> list[Path]:
    """Generate one legacy MONAN precipitation figure per configured hour.

    Parameters
    ----------
    output_dir:
        Directory where the figures will be saved. When omitted, the figures
        are written into `tests/output/legacy_monan_precipitation`.
    ...
    try:
        for time_value in _build_hourly_times(START_TIME, END_TIME):
            figure = build_legacy_monan_precipitation_figure(
                date=time_value,
...
def main() -> None:
    """Generate all legacy MONAN precipitation figures."""
    output_paths = generate_legacy_monan_precipitation_figures()
...
def _slugify_time(time: np.datetime64) -> str:
    ...
    return np.datetime_as_string(time, unit="m").replace(":", "")
